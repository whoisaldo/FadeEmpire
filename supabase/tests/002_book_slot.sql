...
-- ---------- Happy path ----------
select lives_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(3), '11:00', 'Tap Customer', '5551110001') $$,
  'larry books a Wednesday 11:00 hair cut'
);
select is(
...
select lives_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(2), '11:00', 'Tue Customer', '5551110004') $$,
...
select lives_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(0), '12:00', 'Sun Regular', '5551110007') $$,
...
select is(
  (select max(total_price_cents)::int
     from book_slot('larry', 'hair-cut', tap_next_dow(3), '12:00', 'Price Check', '5551110019',
                    array['beard', 'facial'])),
  6000,
...
select is(
  (select max(total_price_cents)::int
     from book_slot('larry', 'hair-cut', tap_next_dow(3), '12:30', 'Bogus Addon', '5551110020',
...
select is(
  (select count(*)::int
     from book_slot('larry', 'vip-haircut', tap_next_dow(3), '14:00', 'Vip Two', '5551110021')),
...
select lives_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(3), '15:30', 'Blocker', '5551110023') $$,
  'a regular cut holds 15:30'
);
select throws_ok(
  $$ select * from book_slot('larry', 'vip-haircut', tap_next_dow(3), '15:00', 'Vip Overlap', '5551110024') $$,
  '23505', 'slot_taken',
...
select throws_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(5), '11:00', 'Holiday Try', '5551110025') $$,
  '22023', 'barber_closed',
...
select throws_ok(
  $$ select * from book_slot('larry', 'hair-cut', tap_next_dow(4), '14:00', 'One Too Many', '5551110026') $$,
